...
export function MessageComponent({ message, onCompanyClick }: MessageProps) {
  ...
  // 加粗 Score/Capabilities/Content 字段名
  function renderWithBoldFields(text: string) {
    if (!text) return null;
    let html = text
      .replace(/(Score:)/g, '<strong>$1</strong>')
      .replace(/(Capabilities:)/g, '<strong>$1</strong>')
      .replace(/(Content:)/g, '<strong>$1</strong>');
    // 兼容 markdown **加粗**
              ...
              {content.type === "text" ? ( 
                <Card className={cn(
                  ...
                  <div className="text-sm whitespace-pre-wrap leading-relaxed"> 
                    {isUser
                      ? content.content
                      : renderWithBoldFields(content.content)}
                  </div> 
                </Card>
              ) : content.type === "image" ? (
